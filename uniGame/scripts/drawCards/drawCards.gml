...
function drawCards(num){
	for(i = 0; i < num; i++)
	{
		if(array_length(global.player_deck) > 0){
			...
			if(global.player_hand[i] == noone)
			{
				...
				global.player_hand[i] = instance_create_layer(920,600,"Instances", global.player_hand[i]);
	...
	for(i = 0; i < noRemaningCards; i++){
		...
		global.ai_hand[i] = instance_create_layer(920,1000,"Instances", global.ai_hand[i]);
